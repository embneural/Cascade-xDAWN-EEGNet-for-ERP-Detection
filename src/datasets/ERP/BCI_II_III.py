...
def random_sample(data:dict, k: int):
    ...
    k: int
        number to average to generate new sample
    ...
    '''
    # 从 train_x中随机挑选2个p300， 然后 平均 组成一个新的 p300
    non_idex   = np.where(data['train'][1] == 0)[0]
    p300_index = np.where(data['train'][1] == 1)[0]
    len_num = p300_index.shape[0]*4
    total_x = np.zeros([ len_num, data['train'][0].shape[1], data['train'][0].shape[2] ])
    total_y = np.zeros([len_num])
    
    for i in range(len_num):
        # a,b = np.random.choice(p300_index, 2, replace = False) 
        # # 要想不重复地抽样(不放回抽样)，需要设置replace参数为False
        # new = (data['train'][0][a] + data['train'][0][b])/2.0
        # new = np.expand_dims(new, axis = 0)
        _list = np.random.choice(p300_index, k, replace = False)
        new = 0
        for idx in _list:
            new += data['train'][0][idx]        
        new = np.expand_dims(new/k, axis = 0)
    ...
    data['train'][0] = np.concatenate((data['train'][0], total_x), axis = 0)
    data['train'][1] = np.concatenate((data['train'][1], total_y), axis = 0)
    ratio = Counter( data['train'][1])

    print('train ratio: {}'.format(ratio))
